functions {
  real country_log_lik(int start, int end,
    ...
    for(m in start:end) {
      ...
      log_lik += neg_binomial_2_lpmf(deaths[EpidemicStart[m]:N[m], m]| E_deaths_slice[m_slice, EpidemicStart[m]:N[m]], ophi );
    }
    return(log_lik);
  }
}
...
transformed parameters {
  ...
  // turn this into an array over the countries
  // which swaps the index
  vector[N2] E_deaths[M] = rep_array(rep_vector(0.0, N2), M);
    ...
    
  {
    ...
    for (m in 1:M){
      ...
      E_deaths[m, 1]= 1e-15 * prediction[1,m];
      for (i in 2:N2){
        for(j in 1:(i-1)){
          //E_deaths[i,m] += prediction[j,m] * f[i-j,m] * ifr_noise[m];
          E_deaths[m,i] += prediction[j,m] * f[i-j,m] * ifr_noise[m];
        }
      }
    }
  }
...
model {
  ...
  */
  target += reduce_sum(country_log_lik, E_deaths, 1, deaths, EpidemicStart, N, ophi);
}
