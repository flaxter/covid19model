  ...
  real country_lpdf(real[] mu_slice,
    ...
    for (m in start:end) {
      ...
      log_lik += neg_binomial_2_lpmf(deaths[EpidemicStart[m]:N[m], m] |
                                     E_deaths[EpidemicStart[m]:N[m]], ophi );
    }
...
transformed data {
  // reversed order
  vector[N2] f_rev[M]; 
  // design matrix per country
  matrix[N2, 6] X[M];    
  // design matrix per country for intercept only model
  matrix[N2, 6] X0[M] = rep_array(rep_matrix(0.0, N2, 6), M);    
  vector[N2] SI_rev;
  for(m in 1:M) {
    for(i in 1:N2) {
